...
uint32_t send_size;
uint32_t recv_size;
static std::string recv_root("/home/dji/Documents/Payload-SDK/recv/");
// Fusion* fusion;
Fusion* fusion_inf;
Fusion* fusion_rgb;
...
static void* ConnectTask(void*) {
    ...
    recvBuf = static_cast<uint8_t *>(handler->Malloc(RECV_BUFFER_SIZE));
    if (recvBuf == NULL) {
    ...
    */
    ...
    while (1) {
        if (connected) {
            ...
            retcode = DjiMopChannel_RecvData(s_MopChannelNormalOutHandle, recvBuf,
                                                RECV_BUFFER_SIZE, &realLen);
            if (retcode != DJI_ERROR_SYSTEM_MODULE_CODE_SUCCESS) {
                if (retcode == DJI_ERROR_MOP_CHANNEL_MODULE_CODE_CONNECTION_CLOSE) {
